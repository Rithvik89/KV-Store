...
func markListenerAsNonBlocking(l net.Listener) int {
	...
	fd := file.Fd()
	fmt.Printf("File descriptor for listener is : %d\n", fd)
...
func handleNewConnections(lfd int, kq int) int {
	for {
		...
		fmt.Printf("Accepted new client fd=%d\n", nfd)
	}
}
...
func main() {
	...
	fmt.Printf("KV Store listening on port %d\n", PORT)
			...
			if fd == lfd {
				...
				if ev.Filter == unix.EVFILT_READ {
					...
					if m > 0 {
						...
						// Write the output.
						fmt.Printf("Processed input command successfully %s", string(buf))
					}
					if err != nil {
						// Handle Disconnect.
					}
